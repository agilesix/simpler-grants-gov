...
def form(request):
    ...
    mapping = module.FORM_XML_TRANSFORM_RULES
    ...
    return SimpleNamespace(
        ...
        inputs={paths.render(p) for p in paths.inputs(resolve_jsonschema(module.FORM_JSON_SCHEMA))},
...
def test_the_record_agrees_with_what_can_actually_be_read(form):
...
def test_every_target_names_an_element_the_schema_declares(form):
...
def test_every_element_the_schema_requires_is_mapped(form):
...
def test_mapped_elements_follow_the_schema_sequence(form):
    ...
    ordered = [
        path for path in form.targets if path in form.declared and form.declared[path].position >= 0
    ]
    out_of_order = []
...
def test_every_response_field_reaches_an_element(form):
...
def test_every_rule_reads_a_field_the_form_has(form):
...
def test_no_field_offers_a_value_its_element_rejects(form):
...
def test_no_field_accepts_a_value_its_element_cannot_carry(form):
    ...
    for path, source, governed, element in _governed(form):
        for keyword, direction in LOOSER.items():
            ...
            if _reconciled(form, path, keyword) or _gap(form, path, keyword):
                continue
            if keyword not in governed:
                looser.append(f"{wire.render(path)} <- {source}: no {keyword}, element {limit}")
            elif (
                (governed[keyword] > limit) if direction == "above" else (governed[keyword] < limit)
            ):
                looser.append(
                    f"{wire.render(path)} <- {source}: {keyword} {governed[keyword]} "
                    f"against element {limit}"
                )
    assert not looser, (
...
def test_every_field_can_become_the_type_its_element_expects(form):
